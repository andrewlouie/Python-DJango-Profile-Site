...
try {
	...
	$sql = "SELECT IIF(prov='Other',country,prov) prov FROM users WHERE email = '" . $_COOKIE['username'] . "';";
...
try {
    $dbh = new PDO('sqlsrv:server=127.0.0.1;Database=toys');
    foreach($dbh->query("SELECT title,iif(charindex(',', pics)=0,pics,left(pics,charindex(',', pics)-1)) pics,IIF(saleprice IS NULL,price,saleprice) price FROM TOYS WHERE item = " . $_SESSION['cart'][$i]) as $row) {
	echo "<tr><td>";
	echo "<img src='images/";
	echo $row['pics'];
	echo "'></td><td>";
