...
<head>
	...
	function provSel(pro) {
		if (pro == "Other") {
			...
			document.getElementById('country-selector').value = "Canada";
		}
	}
	</script>
</head>

<body<?php if (isset($_POST['country'])) echo ' onLoad="test(\'' . $_POST['country'] . '\', \'' . $_POST['prov'] . '\')"' ?>>
...
if (isset($_POST['submitted'])) {
	$error=0;
	if (!isset($_POST['firstname']) || $_POST['firstname'] == "") { echo "First name is required<br>"; $error=1; }
	if (!isset($_POST['lastname']) || $_POST['lastname'] == "") { echo "Last name is required<br>"; $error=1; }
	if (!isset($_POST['address']) || $_POST['address'] == "") { echo "Street address is required<br>"; $error=1; }
	if (!isset($_POST['city']) || $_POST['city'] == "") { echo "City is required<br>"; $error=1; }
	if (!isset($_POST['prov']) || $_POST['prov'] == "") { echo "Province is required<br>"; $error=1; }
	if (!isset($_POST['prov']) || ($_POST['prov'] == "Other" && (!isset($_POST['other']) || $_POST['other'] == ""))) { echo  "Other is required if no province is selected<br>"; $error=1; }
	if (!isset($_POST['postal']) || $_POST['postal'] == "") { echo "Postal/Zip code is required<br>"; $error=1; }
	if (!isset($_POST['country']) || $_POST['country'] == "") { echo "Country is required<br>"; $error=1; }
	if (!isset($_POST['password']) || $_POST['password'] == "") { echo "Password is required<br>"; $error=1; }
	if (!isset($_POST['verifypw']) || $_POST['verifypw'] == "") { echo "Verify password is required<br>"; $error=1; }
	if (isset($_POST['password']) && isset($_POST['password']) && $_POST['password'] != "" && $_POST['verifypw'] != "" &&$_POST['password'] != $_POST['verifypw']) { echo "Password does not match<br>"; $error=1; }
	if (!isset($_POST['email']) || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) { echo "Email address is invalid"; $error=1; }
	if ($error==0) {
		try {
	...
	if ($error==0) {
		try {
			$dbh = new PDO('sqlsrv:server=127.0.0.1;Database=toys');
			$sql = "INSERT INTO users (user_id, email, password, firstname, lastname, address, city, prov, postal, country, datecreated, ip, other) VALUES (NULL, '" . $_POST['email']. "', '" . md5($_POST['password']) . "', '" . $_POST['firstname'] . "', '" . $_POST['lastname'] . "', '" . $_POST['address'] . "', '" . $_POST['city'] . "', '" . $_POST['prov'] . "', '" . $_POST['postal'] . "', '" . $_POST['country'] . "', CURDATE(), '" . $_SERVER['REMOTE_ADDR'] . "', '" . $_POST['other'] . "');";
			$sth = $dbh->prepare($sql);	
			$sth->execute();
			$dbh = null;
...
<form class="loginform" method="post" action="newacct.php">
<label>Email address: </label><input name="email" type="text" class="text" id="email" placeholder="Email address"
...
<label>First Name: </label><input name="firstname" type="text" class="text" id="firstname"<?php if (isset($_POST['firstname'])) echo " value='" . $_POST['firstname'] . "'" ?>><br>
<label>Last Name: </label><input name="lastname" type="text" class="text" id="lastname"<?php if (isset($_POST['lastname'])) echo " value='" . $_POST['lastname'] . "'" ?>><br>
<label>Street Address: </label><input name="address" type="text" class="text" id="address"<?php if (isset($_POST['address'])) echo " value='" . $_POST['address'] . "'" ?>><br>
<label>City: </label><input name="city" type="text" class="text" id="city"<?php if (isset($_POST['city'])) echo " value='" . $_POST['city'] . "'" ?>><br>
...
<br>
<label>Other: </label><input name="other" type="text" class="text" id="other"<?php if (isset($_POST['other'])) echo " value='" . $_POST['other'] . "'" ?>><br>
    <label>Postal/Zip Code: </label><input name="postal" type="text" class="text" id="postal"<?php if (isset($_POST['postal'])) echo " value='" . $_POST['postal'] . "'" ?>><br>
    <label>Country: </label>
...
<label></label><input type="submit" class="aabtn" value="Sign Up"> <a href="login.php">Already have an account?</a>
</form>
</div></div>
